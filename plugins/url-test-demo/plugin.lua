plugin = {
    id = "url-test-demo",
    name = "URL Test Demo",
    description = "Test plugin for the Install from URL flow (forked from system-info)",
    version = "1.0.2",
    author = "Cleat",
    icon = "i",
    category = "System",
...
function transform(raw, cfg)
    ...
    -- character class containing just `-` (Lua deduplicates), so it
    -- split on any hyphen — breaking on kernel versions like "6.8.0-..."
    -- in `uname -a` output.
    local sections = {}
    local pos = 1
    while true do
